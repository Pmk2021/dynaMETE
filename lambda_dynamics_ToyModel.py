...
def get_new_dl_matrix_vector(l,s,p,ds, d_d):
    '''
    Calculates the matrix and vector of covariances needed for lambda dynamics
    l are lambdas as an array, so l[0] is lambda_1 etc.
    s are state variables, call S, N
    ...
    covnf = rf.cov(lambda n,s,p: n,rf.f,l,s,p,z)
    covff = rf.cov(rf.f,rf.f,l,s,p,z)
    cov_ndfdd = rf.cov(lambda n,s,p:n,rf.dfdd,l,s,p,z)
    cov_f_dfdd = rf.cov(rf.f,rf.dfdd,l,s,p,z)
    var_dfdd = rf.cov(rf.dfdd,rf.dfdd,l,s,p,z)
        
    # Then put them in rows
    r1 = np.array([covnn,covnf])
    r2 = np.array([covnf,covff])
    ...
    # Vectors, including normalization in proper spot.
    v2 = -1 * (d_d * rf.dfdd_sum(l,s,p,z))/s['S'] + var_dfdd * d_d + -1 * l[1] * cov_f_dfdd * d_d
    v1 = -1 * l[1] * cov_ndfdd * d_d
